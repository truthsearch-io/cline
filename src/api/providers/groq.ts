...
const MODEL_FAMILIES: Record<string, GroqModelFamily> = {
	...
	deepseek: {
		name: "DeepSeek",
		supportedFeatures: { streaming: true, temperature: true, vision: false, tools: true },
		optimalTemperature: { plan: 0.2, act: 0.1 },
		maxTokensOverride: 8192,
	},
	// Google Gemma Family
	gemma: {
...
export class GroqHandler implements ApiHandler {
	...
	private getOptimalTemperature(modelFamily: GroqModelFamily, isPlanMode?: boolean): number {
		// Check if temperature is explicitly configured (from user settings)
		// Note: ModelInfo doesn't have temperature, so we'll use a different approach
		// Temperature might be passed via options or we use optimal values
