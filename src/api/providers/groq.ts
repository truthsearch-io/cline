...
export class GroqHandler implements ApiHandler {
	...
	/**
	 * Determines if R1 message format is required based on model family
	 */
	private isR1FormatRequired(modelFamily: GroqModelFamily): boolean {
		// Use R1 format for qwq, qwen, and deepseek model families
		return modelFamily.name === "QwQ Reasoning" || modelFamily.name === "Qwen" || modelFamily.name === "DeepSeek"
	}
	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		// Determine if R1 format is required for this model family
		const useR1Format = this.isR1FormatRequired(modelFamily)

		let openAiMessages: OpenAI.Chat.ChatCompletionMessageParam[]

		if (useR1Format) {
			// For R1 format: convert system prompt to user message and use R1 format conversion
			const allMessages: Anthropic.Messages.MessageParam[] = [{ role: "user", content: systemPrompt }, ...messages]
			openAiMessages = convertToR1Format(allMessages)
		} else {
			// Standard format: separate system message
